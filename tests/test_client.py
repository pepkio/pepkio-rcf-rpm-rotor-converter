...
from pepkio_rcf_rpm_rotor_converter.config import TOOL_ID
from pepkio_rcf_rpm_rotor_converter.exceptions import PepkioAPIError
from pepkio_rcf_rpm_rotor_converter.models import RunResult
...
def test_get_manifest_and_cache(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert m1["tool_id"] == "rcf-rpm-rotor-converter"
        assert m1 is m2
    assert len(calls) == 1
...
def test_list_examples_and_get_example_input(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert inp["mode"] == "convert"
        assert inp["direction"] == "rpm_to_rcf"
...
def test_run_sends_correct_request(mock_manifest, mock_run_response):
    ...
    with _make_client(handler) as client:
        result = client.run(client.get_example_input("rpm_to_rcf_10k"), label="pytest")

    assert captured["path"] == f"/api/tools/v1/tools/{TOOL_ID}/run"
    assert captured["auth"] == "Bearer test-key"
    assert captured["body"]["input"]["mode"] == "convert"
    assert captured["body"]["options"]["label"] == "pytest"
    assert result.status == "completed"
    assert result.result["result"]["rcf"]["rmax_rcf"] == 6708
